...
import { NET_KINDS, NET_KINDS_ARRAY } from '../../primitives/messages/NetMessage/NET_KINDS.js';
 ...
 */
function deserializeArray(bytes) {
    ...
    for(let i = 0; i < itemsAmount; i++) {
        ...
        switch(fieldType){
            ...
            case "boolean":
                ...
                break;
            case "array":
                ({value: fieldValue, length: fieldValueBytesLength} = deserializeArray(bytes.subarray(offset)));
                offset += fieldValueBytesLength;
                break;
            case "object":
                ({value: fieldValue, length: fieldValueBytesLength} = deserializeObject(bytes.subarray(offset)));
                offset += fieldValueBytesLength;
                break;
            default:
 ...
 */
function deserializeObject(inputArray) {
